  ...
  <div class="wrapper">
    ...
    <div class="content" v-if="accesstoken">
      ...
      <mu-list-item class="list-item" title="未读消息" v-if="readMsg.hasnot_read_messages" toggleNested :open="false">
        ...
        <mu-list-item class="list-item-content" :to="{path:'/qjhome/dist/content',query:{id:item.topic.id}}" v-for="item in readMsg.hasnot_read_messages" :key="item.id" slot="nested" :title="item.title">
      ...
      <mu-list-item class="list-item" v-if="readMsg.has_read_messages" title="已读消息" toggleNested :open="false">
        ...
        <mu-list-item class="list-item-content" :to="{path:'/qjhome/dist/content',query:{id:item.topic.id}}" v-for="item in readMsg.has_read_messages" :key="item.id" slot="nested" :title="item.title">
    ...
    <div v-if="!accesstoken" class="message-tip">
      <mu-raised-button to="/qjhome/dist/login" label="请先登录" class="demo-raised-button" primary/>
    </div>
    <FooterNav></FooterNav>
    </div>
  </div>
